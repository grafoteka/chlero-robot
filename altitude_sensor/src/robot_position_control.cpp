...
*/
        ...
        if((lResult = ClearErrors(&ulErrorCode))!=MMC_SUCCESS)
        {
        ...
        //unsigned int ulErrorCode = 0;
        int p_usNodeId = 6;
        HANDLE p_DeviceHandle = g_pKeyHandle;

        int speed = 330;
        int accel = 10000;
        int decel = 10000;

        VCS_ActivateProfilePositionMode(p_DeviceHandle, p_usNodeId, &ulErrorCode);
        VCS_ActivateProfilePositionMode(p_DeviceHandle, 2, &ulErrorCode);
        VCS_SetPositionProfile(p_DeviceHandle, p_usNodeId, speed, accel, decel, &ulErrorCode);
        VCS_MoveToPosition(p_DeviceHandle, p_usNodeId, 132000, 0, 1, &ulErrorCode);
        ...
        {
            int position_raw;
            VCS_GetPositionIs(p_DeviceHandle, 6, &position_raw, &ulErrorCode);

            ROS_INFO("Valor: %d", position_raw);*/
            /*p_usNodeId = 6;
            speed = 3000;
            VCS_SetPositionProfile(p_DeviceHandle, p_usNodeId, speed, accel, decel, &ulErrorCode);
            if(VCS_MoveToPosition(p_DeviceHandle, p_usNodeId, 111000, 0, 1, &ulErrorCode) == 0)
            {
                ROS_INFO("VCS_MoveToPosition");//, lResult, &ulErrorCode);
                lResult = MMC_FAILED;
                break;
            }
            ...
            if(lResult == MMC_SUCCESS)
            {
                ...
                if(VCS_HaltPositionMovement(p_DeviceHandle, p_usNodeId, &ulErrorCode) == 0)
                {
                        ROS_INFO("VCS_HaltPositionMovement");//, lResult, &ulErrorCode);
                        lResult = MMC_FAILED;
                }
            }*/
            /*if(lResult)
            {
                speed = 300;
                VCS_SetPositionProfile(p_DeviceHandle, p_usNodeId, speed, accel, decel, &ulErrorCode);
                lResult = VCS_MoveToPosition(p_DeviceHandle, p_usNodeId, 22000, 0, 0, &ulErrorCode);
            }*/
    ...
    while(ros::ok())
    {
        ...
        // Motor 6 activate profile position mode (PPM) #pag 5-72
        /*static bool flag = true;
        if(flag)
        {
            static int lResult = MMC_SUCCESS;
            unsigned int ulErrorCode = 0;
            int p_usNodeId = 6;
            HANDLE p_DeviceHandle = g_pKeyHandle;
            if(VCS_ActivateProfilePositionMode(p_DeviceHandle, p_usNodeId, &ulErrorCode) == 0)
            {
            ...
            //Al salir de este if, MMC_FAILED = 1
        }*/

        // Motor 6 move to desired position
        // (HANDLE KeyHandle, WORD NodeId, long TargetPosition, BOOL Absolute, BOOL Immediately, DWORD* pErrorCode)
        //static bool flag_order = true;
        /*static int iterator;
        ROS_INFO("iterator_prev = %d", iterator);
        //if(flag_order)
        if(iterator == 0)
        {
            ...
            HANDLE p_DeviceHandle = g_pKeyHandle;
            int p_usNodeId = 6;
            bool absolute = false; // movimiento de posicion relativo
            bool inmediately = true; // movimiento inmediato
            unsigned int ulErrorCode = 0;
            ...
            //ROS_INFO("Move en posicion");
            VCS_ActivateProfilePositionMode(p_DeviceHandle, p_usNodeId, &ulErrorCode);


            //VCS_MoveToPosition(p_DeviceHandle, p_usNodeId, TargetPosition, absolute, inmediately, &ulErrorCode);
            if(VCS_MoveToPosition(p_DeviceHandle, p_usNodeId, TargetPosition, 0, 1, &ulErrorCode) == 0)
            {
                lResult = MMC_FAILED;
            ...
            int leg_actual_pos_raw;
            VCS_GetPositionIs(p_DeviceHandle, p_usNodeId, &leg_actual_pos_raw, &ulErrorCode);

            ROS_INFO("Actual position: %d", leg_actual_pos_raw);*/
