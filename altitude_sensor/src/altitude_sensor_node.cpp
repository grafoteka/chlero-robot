...
int main(int argc, char** argv) {
    ...
    while(ros::ok()) {
        ...
        for(int i = 0; i <= 5; i++)
        {
            unsigned int p_usNodeId = i+1;
            unsigned int ulErrorCode = 0;
            HANDLE p_DeviceHandle;

            short leg_actual_current_raw;
            VCS_GetCurrentIs(p_DeviceHandle, p_usNodeId, &leg_actual_current_raw, &ulErrorCode);
            leg_actual_current[i] = leg_actual_current_raw / 1000.0; //mA -> A
            double torque_constant = 2.6;
            leg_actual_effort[i] = leg_actual_current[i] * torque_constant;

            ROS_INFO("Motor: %i -- Corriente: %.2f [mA] -- Torque: %.2f", p_usNodeId, leg_actual_current[i], leg_actual_effort[i]);
        }
